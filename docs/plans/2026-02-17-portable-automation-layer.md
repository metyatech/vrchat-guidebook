...
**Goal:** Build a reusable, profile-driven automation layer that can regenerate guidebook
docs/images/videos across different avatar states, Unity versions, and tool versions without
rewriting step logic.

**Architecture:** Add a repository-local portable layer on top of `automation-scenario` with four
parts: profile definitions, reusable blueprints, capability/version rules, and a compiler/runner
CLI. Compilation resolves templates and conditions into deterministic `.scenario.json` files and
manifest output. Execution uses compiled scenarios to call `automation-scenario run-scenario`
consistently.
...
Run: `node --test tests/automation/portable.contracts.test.cjs` Expected: FAIL because
parser/validator does not exist yet.
...
Run: `node --test tests/automation/portable.contracts.test.cjs` Expected: PASS.
...
Run: `node --test tests/automation/portable.compiler.test.cjs` Expected: FAIL with missing
module/function.
...
Run: `node --test tests/automation/portable.compiler.test.cjs` Expected: PASS.
...
Run: `node --test tests/automation/portable.runner.test.cjs` Expected: FAIL.
...
Run: `node --test tests/automation/portable.runner.test.cjs` Expected: PASS.
...
Run: `npm run test:automation` Expected: FAIL before integration wiring.
...
Run: `npm run test:automation` Expected: PASS.
...
Run: `git status --short` Expected: only source/docs/test changes.
