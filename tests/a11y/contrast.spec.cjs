...
const INTERACTIVE_SELECTOR =
  '#VPContent a, #VPContent button, #VPContent [role="button"], .VPHero a';
  ...
  const issues = await page.evaluate(
    ({
      ...
      for (const candidate of candidates) {
        ...
        if (
          !isInteractive &&
          ["h1", "h2", "h3", "h4", "h5", "h6", "code", "pre", "kbd", "hr"].includes(tagName)
        ) {
          continue;
        }
...
test("color contrast is valid for light/dark and interactive states", async ({
  browser,
  request
}) => {
